...
class character {
	...
	function is_owner () {
		...
		$count = Database::fetchValue(
			"SELECT COUNT(`charId`) FROM `characters` WHERE `account_name` = ? AND `charId` = ?",
			[ACCOUNT::load()->getLogin(), $this->charId],
			$this->worldId
		);
	...
	function load() {
		Database::execute(
			"SET NAMES '" . CONFIG::g()->core_iso_type . "'",
			[],
			$this->worldId
		);

		$row = Database::fetchOne(
			"SELECT `char_name`, `base_class`, `sex`, `accesslevel`, `x`, `y`, `online`, `clanid`, `level`, `karma` FROM `characters` WHERE `charId` = ? LIMIT 1",
			[$this->charId],
			$this->worldId
		);
	...
	function reload($chp) {
		$allowedColumns = ['char_name', 'base_class', 'sex', 'accesslevel', 'x', 'y', 'online', 'clanid', 'level', 'karma'];
		if (!in_array($chp, $allowedColumns, true)) {
			return false;
		}

		$row = Database::fetchOne(
			"SELECT `{$chp}` FROM `characters` WHERE `charId` = ? LIMIT 1",
			[$this->charId],
			$this->worldId
		);
	...
	function is_hero () {
		...
		$count = Database::fetchValue(
			"SELECT COUNT(`charId`) FROM `heroes` WHERE `charId` = ?",
			[$this->charId],
			$this->worldId
		);
	...
	function allow_fix($unstuck = false) {
		...
		$count = Database::fetchValue(
			"SELECT COUNT(`account_name`) FROM `account_data` WHERE `var` = ? AND `account_name` = ? AND `value` > ?",
			['last_'.$last, $this->charId, (time()-(CONFIG::g()->service_fix_time * 3600))],
			$this->worldId
		);
	...
	function fix() {
		...
		Database::execute(
			"UPDATE `characters` SET `x` = ?, `y` = ?, `z` = ? WHERE `charId` = ?",
			[$t[0], $t[1], $t[2], $this->charId],
			$this->worldId
		);

		DEBUG::add('Fix shortcuts of character');
		Database::execute(
			"DELETE FROM `character_shortcuts` WHERE `charId` = ?",
			[$this->charId],
			$this->worldId
		);
		...
		Database::execute(
			"REPLACE INTO `account_data` (`account_name`, `var`, `value`) VALUES (?, 'last_fix', ?)",
			[$this->charId, time()],
			$this->worldId
		);
	...
	function unstuck () {
		...
		DEBUG::add('Add a tag for prevent abus');
		Database::execute(
			"REPLACE INTO `account_data` (`account_name`, `var`, `value`) VALUES (?, 'last_unstuck', ?)",
			[$this->charId, time()],
			$this->worldId
		);
	...
	function name($new_name) {
		...
		Database::execute(
			"UPDATE `characters` SET `char_name` = ? WHERE `charId` = ?",
			[$new_name, $this->charId],
			$this->worldId
		);

		DEBUG::add('Add a tag for prevent abus');
		Database::execute(
			"REPLACE INTO `account_data` (`account_name`, `var`, `value`) VALUES (?, 'previous_name', ?)",
			[$this->charId, $this->char_name],
			$this->worldId
		);
	...
	function can_change_name ($new_name = null, $test = null) {
		...
		$count = Database::fetchValue(
			"SELECT COUNT(`account_name`) FROM `account_data` WHERE `var` = 'previous_name' AND `account_name` = ?",
			[$this->charId],
			$this->worldId
		);
		...
		$count = Database::fetchValue(
			"SELECT COUNT(`char_name`) FROM `characters` WHERE `char_name` = ?",
			[$new_name],
			$this->worldId
		);
	...
	function sex() {
		...
		foreach ($items as $id) {												// Foreach items listed set in inventory if they exist.
			Database::execute(
				"UPDATE `items` SET `loc` = 'INVENTORY' WHERE `owner_id` = ? AND `item_id` = ?",
				[$this->charId, $id],
				$this->worldId
			);
		}
		...
		Database::execute(
			"UPDATE `characters` SET `sex` = ?, `face` = 0, `hairStyle` = 0, `hairColor` = 0 WHERE `charId` = ?",
			[$this->sex, $this->charId],
			$this->worldId
		);

		DEBUG::add('Add a tag for prevent abus');
		Database::execute(
			"REPLACE INTO `account_data` (`account_name`, `var`, `value`) VALUES (?, 'last_gender_change', ?)",
			[$this->charId, time()],
			$this->worldId
		);
	...
	function can_change_gender ($test = null) {
		...
		$count = Database::fetchValue(
			"SELECT COUNT(`account_name`) FROM `account_data` WHERE `var` = 'last_gender_change' AND `account_name` = ? AND `value` > ?",
			[$this->charId, (time()-(CONFIG::g()->service_sex_time * 24 * 3600))],
			$this->worldId
		);
	...
	function mapRegionTable() {
		$rows = Database::fetchAll(
			"SELECT `region`, `sec0`, `sec1`, `sec2`, `sec3`, `sec4`, `sec5`, `sec6`, `sec7`, `sec8`, `sec9`, `sec10`, `sec11`, `sec12`, `sec13`, `sec14`, `sec15` FROM `mapregion`",
			[],
			$this->worldId
		);
		...
		foreach ($rows as $row) {
			$region = $row['region'];
			for ($j = 0; $j < 16; $j++)
				$this->regions[$j][$region] = $row['sec'.$j];
		}
	}
